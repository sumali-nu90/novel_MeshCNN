...
class VertexMeshConv(nn.Module):
    """Computes convolution between vertices and their 4 incident (1-ring) vertex neighbors.
    ...
    Shape:
        - Input:
            - x: (batch_size, in_channels, vertices)
            - mesh: list of mesh data-structure (len(mesh) == batch_size)
        - Output:
            - Output tensor: (batch_size, out_channels, vertices)
